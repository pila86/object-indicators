...
Процедура ОбновитьДанныеСправочника() Экспорт
	...
	ЗаписатьКоллекциюСвойствоОъектовМетаданных(КоллекцияСвойств);
...
Процедура ЗаписатьКоллекциюСвойствоОъектовМетаданных(КоллекцияСвойств)
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("КоллекцияСвойств",	ПодготовленнаяКоллекция);
	...
	ВыборкаСоздать				= РезультатыЗапроса[3].Выбрать();
	ВыборкаОбновить				= РезультатыЗапроса[4].Выбрать();
	ВыборкаПометитьНаУдаление	= РезультатыЗапроса[5].Выбрать();
	ВыборкаТипыСвойств			= РезультатыЗапроса[6].Выбрать();
